...
async function batchConvert(input: string, options: BatchOptions): Promise<void> {
  ...
  for (let i = 0; i < files.length; i += parallelLimit) {
    ...
    const batchPromises = batch.map(file => convertSingleFile(file, input, options, converter));
...
async function findFiles(input: string, options: BatchOptions): Promise<string[]> {
  try {
...
function generateOutputPath(filePath: string, inputBase: string, outputBase: string): string {
  try {
    ...
    if (stats.isFile()) {
      ...
      const outputFileName = basename(relativePath, extname(relativePath)) + '.ts';
...
function displayResults(results: ConversionResult[], totalDuration: number, options: BatchOptions): void {
  ...
  if (options.summary && successful.length > 0) {
    ...
    successful.forEach(result => {
      ...
      if (options.verbose) {
        console.log(chalk.gray(`    Nodes: ${result.nodeCount}, Duration: ${result.duration}ms`));
  ...
  if (successful.length > 0) {
    ...
    console.log(chalk.gray(`  Average nodes per file: ${Math.round(avgNodes)}`));
    console.log(chalk.gray(`  Conversion rate: ${Math.round(successful.length / (totalDuration / 1000))} files/sec`));
  }
}
