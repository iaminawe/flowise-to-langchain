...
export function createRunCommand(): Command {
  return new Command('run')
    ...
    .option('-o, --output <path>', 'Output directory for generated files', './temp')
...
async function runWorkflow(file: string, input: string = '', options: RunOptions): Promise<void> {
  ...
  try {
    ...
    const mainFile = writtenFiles.find(f => f.endsWith('.ts')) || writtenFiles[0];
    ...
    if (!options.keepFiles && tempDir !== options.output) {
      try {
        ...
        console.warn(chalk.yellow('⚠️  Could not clean up temporary files:'), error);
      }
    } else {
...
async function createRunnerScript(mainFile: string, input: string, options: RunOptions): Promise<string> {
