...
export async function fixFlowiseIssues(flowiseData: any, validationResult: ValidationResult): Promise<any | null> {
      ...
      switch (error.code) {
        ...
        case 'MISSING_EDGE_PROPERTY':
          if (fixedData.edges && Array.isArray(fixedData.edges)) {
            ...
            fixedData.edges = fixedData.edges.filter((edge: any, index: number) => {
              if (error.path?.includes(`[${index}]`)) {
                if (!edge.source || !edge.target) {
                  console.log(`⚠️  Removed invalid edge ${index} (missing source/target)`);
                  fixesApplied++;
                  return false;
                }
              }
        ...
        case 'INVALID_EDGE_REFERENCE':
          if (fixedData.edges && fixedData.nodes && Array.isArray(fixedData.edges) && Array.isArray(fixedData.nodes)) {
            const nodeIds = fixedData.nodes.map((n: any) => n.id).filter(Boolean);
            
            // Remove edges with invalid references
            fixedData.edges = fixedData.edges.filter((edge: any) => {
              if (!nodeIds.includes(edge.source) || !nodeIds.includes(edge.target)) {
                console.log(`⚠️  Removed edge with invalid reference: ${edge.source} -> ${edge.target}`);
                fixesApplied++;
                return false;
              }
              return true;
        ...
        case 'EMPTY_NODE_DATA':
          if (fixedData.nodes && Array.isArray(fixedData.nodes)) {
            fixedData.nodes.forEach((node: any, index: number) => {
              if (error.path?.includes(`[${index}]`) && node.data && Object.keys(node.data).length === 0) {
                // Add some default data based on node type
                node.data = getDefaultNodeData(node.type);
                fixesApplied++;
                console.log(`✅ Added default data to node ${index} (${node.type})`);
              }
            });
          }
          break;
        ...
        case 'INVALID_NODE_ID':
          if (fixedData.nodes && Array.isArray(fixedData.nodes)) {
            fixedData.nodes.forEach((node: any, index: number) => {
              if (error.path?.includes(`[${index}]`) && (!node.id || node.id.length === 0)) {
...
function applyAdditionalFixes(data: any): any {
  ...
  try {
    ...
    console.log('Warning: Error applying additional fixes:', (error as Error).message);
    return data;
  }
}
...
export function generateFixReport(originalData: any, fixedData: any): string {
  ...
  if (originalData.nodes?.length !== fixedData.nodes?.length) {
    report.fixes.push(`Node count changed: ${originalData.nodes?.length || 0} -> ${fixedData.nodes?.length || 0}`);
  }

  if (originalData.edges?.length !== fixedData.edges?.length) {
    report.fixes.push(`Edge count changed: ${originalData.edges?.length || 0} -> ${fixedData.edges?.length || 0}`);
  }
