...
export class TypeScriptEmitter {
  ...
  async generateCode(graph: IRGraph, context: GenerationContext): Promise<CodeGenerationResult> {
    ...
    const allDependencies = this.extractDependencies(organizedFragments, context);
   ...
   */
  private generateCodeFragments(graph: IRGraph, context: GenerationContext): CodeFragment[] {
    ...
    if (context.includeLangfuse) {
      fragments.push(...this.langfuseIntegrator.generateIntegrationFragments(graph, context));
    }
   ...
   */
  private organizeFragments(fragments: CodeFragment[]): Map<string, CodeFragment[]> {
    const organized = new Map<string, CodeFragment[]>();
    
    // Initialize categories
    const categories = ['import', 'declaration', 'initialization', 'execution', 'export'];
  ...
  private async generateFiles(
    ...
    files.push(await this.generatePackageJson(organizedFragments, graph, context));
  ...
  private async generateMainFile(
    ...
    const content = this.templateEngine.renderTemplate('mainFile', {
      ...
      description: graph.metadata.description || 'Generated LangChain application'
    });

    const formattedContent = await this.codeFormatter.format(content, 'typescript');
   ...
   */
  private async generateTypesFile(graph: IRGraph, context: GenerationContext): Promise<GeneratedFile> {
   ...
   * Generate configuration file
   */
  private async generateConfigFile(graph: IRGraph, context: GenerationContext): Promise<GeneratedFile> {
   ...
   */
  private async generateEnvironmentFile(graph: IRGraph, context: GenerationContext): Promise<GeneratedFile> {
    const envVars = this.extractEnvironmentVariables(graph);
    
    const content = envVars.map(variable => 
   ...
   * Generate test files
   */
  private async generateTestFiles(graph: IRGraph, context: GenerationContext): Promise<GeneratedFile[]> {
    ...
    const formattedTestContent = await this.codeFormatter.format(mainTestContent, 'typescript');
    testFiles.push({
  ...
  private sortNodesForExecution(graph: IRGraph): IRNode[] {
    ...
    const entryPoints = graph.nodes.filter(node => 
      !graph.connections.some(c => c.target === node.id)
    );
  ...
  private isAsyncNode(node: IRNode): boolean {
    ...
    const asyncCategories = ['llm', 'chain', 'agent', 'tool', 'vectorstore', 'retriever'];
  ...
  private hasAsyncOperations(organizedFragments: Map<string, CodeFragment[]>): boolean {
  ...
  private calculateMetadata(
    ...
    if (dependencies.includes('@langchain/core')) features.push('LangChain Core');
   ...
   */
  private extractFileDependencies(organizedFragments: Map<string, CodeFragment[]>): string[] {
   ...
   */
  private extractFileExports(organizedFragments: Map<string, CodeFragment[]>): string[] {
  ...
  private extractWarnings(graph: IRGraph): string[] {
    ...
    const unsupportedNodes = graph.nodes.filter(node => !this.getNodeConverter(node));
    if (unsupportedNodes.length > 0) {
      warnings.push(`${unsupportedNodes.length} unsupported node type(s) found`);
    }
  ...
  private initializeNodeConverters(): void {
    ...
    import('./converters/llm-converter.js').then(module => {
    ...
    import('./converters/chain-converter.js').then(module => {
    ...
    import('./converters/prompt-converter.js').then(module => {
    ...
    import('./converters/memory-converter.js').then(module => {
    ...
    import('./converters/tool-converter.js').then(module => {
