...
abstract class BaseAgentFlowV2Converter extends BaseConverter {
  ...
  convert(node: IRNode, context: GenerationContext): CodeFragment[] {
    ...
    if (Object.keys(config.state).length > 0) {
      const stateCode = this.generateStateManagementCode(config.state, variableName);
      fragments.push(
        this.createCodeFragment(
          `${node.id}_state`,
    ...
    const postInitCode = this.generatePostInitializationCode(node, context, variableName);
    fragments.push(
      this.createCodeFragment(
        `${node.id}_post_init`,
        'initialization',
  ...
  protected generateStateManagementCode(
    ...
    const stateInit = stateEntries.map(([key, value]) => {
   ...
   */
  protected getAdditionalImports(_node: IRNode, _context: GenerationContext): string[] {
    return [];
  }
...
export class AgentNodeConverter extends BaseAgentFlowV2Converter {
  ...
  override getDependencies(node: IRNode, _context: GenerationContext): string[] {
    ...
    const enableCallbacks = this.getParameterValue(node, 'enableCallbacks', false);
    if (enableCallbacks) {
  ...
  validateConfiguration(node: IRNode): { valid: boolean; errors: string[] } {
    ...
    const maxIterations = this.getParameterValue(node, 'maxIterations', 10);
    if (typeof maxIterations !== 'number' || maxIterations < 1 || maxIterations > 100) {
      errors.push('maxIterations must be a number between 1 and 100');
    }
    
    // Check max execution time
    const maxExecutionTime = this.getParameterValue(node, 'maxExecutionTime', 30000);
  ...
  protected override generateSetupCode(
    ...
    const planningStrategy = this.getParameterValue(node, 'planningStrategy', null);
    const errorHandlingStrategy = this.getParameterValue(node, 'errorHandlingStrategy', null);
    
    if (enableCallbacks) {
      setupLines.push(`// Setup callback manager`);
      setupLines.push(`const ${variableName}_callbackManager = new CallbackManager();`);
    }
    
    if (dynamicTools) {
      setupLines.push(`// Setup dynamic tool loading`);
      setupLines.push(`const ${variableName}_toolLoader = new DynamicToolLoader();`);
    }
    
    if (planningStrategy) {
      setupLines.push(`// Configure planning strategy: ${planningStrategy}`);
      setupLines.push(`const ${variableName}_planner = new PlanningStrategy('${planningStrategy}');`);
    }
    
    if (errorHandlingStrategy) {
      setupLines.push(`// Configure error handling strategy: ${errorHandlingStrategy}`);
      setupLines.push(`const ${variableName}_errorHandler = new ErrorHandler('${errorHandlingStrategy}');`);
    }
...
export class ToolNodeConverter extends BaseAgentFlowV2Converter {
  ...
  protected override getAdditionalImports(node: IRNode, _context: GenerationContext): string[] {
    ...
    if ((inputSchema && Object.keys(inputSchema).length > 0) || (outputSchema && Object.keys(outputSchema).length > 0)) {
    ...
    if (asyncExecution) {
      imports.push('import { AsyncCallbackManagerForToolRun } from "@langchain/core/callbacks/manager";');
    }
    
    // Add error handling imports
    const enableRetry = this.getParameterValue(node, 'enableRetry', false);
    if (enableRetry) {
      imports.push('import { AsyncRetrier } from "@langchain/core/utils/async_caller";');
    }
  ...
  protected override generateSetupCode(
    ...
    if (Object.keys(inputSchema || {}).length > 0) {
      ...
      setupLines.push(`const ${variableName}_inputSchema = z.object(${JSON.stringify(inputSchema, null, 2)});`);
    }
    
    if (Object.keys(outputSchema || {}).length > 0) {
      setupLines.push(`// Output validation schema`);
      setupLines.push(`const ${variableName}_outputSchema = z.object(${JSON.stringify(outputSchema, null, 2)});`);
    }
  ...
  protected extractNodeConfig(node: IRNode): Record<string, unknown> {
    ...
    const validateOutput = this.getParameterValue(node, 'validateOutput', false);
    ...
    if (toolFunction) {
      ...
      if (asyncExecution) {
        funcLines.push(`async (input: string, runManager?: AsyncCallbackManagerForToolRun) => {`);
      } else {
      ...
      if (validateInput && Object.keys(inputSchema || {}).length > 0) {
        ...
        funcLines.push(`  const parsedInput = ${toolName}_inputSchema.parse(JSON.parse(input));`);
      }
      
      // Add logging
      if (enableLogging) {
        funcLines.push(`  console.log('Executing tool: ${toolName}', { input });`);
      }
      
      // Add timeout wrapper
      if (timeout && timeout > 0) {
        funcLines.push(`  const timeoutPromise = new Promise((_, reject) => {`);
        funcLines.push(`    setTimeout(() => reject(new Error('Tool execution timeout')), ${timeout});`);
        funcLines.push(`  });`);
      }
      ...
      if (validateOutput && Object.keys(outputSchema || {}).length > 0) {
        ...
        funcLines.push(`      const validatedResult = ${toolName}_outputSchema.parse(result);`);
        funcLines.push(`      return JSON.stringify(validatedResult);`);
      } else {
        funcLines.push(`      return typeof result === 'string' ? result : JSON.stringify(result);`);
      }
  ...
  protected extractNodeState(node: IRNode): Record<string, unknown> {
    ...
    if (enableCaching) {
      state['cache'] = {
        ...
        keyStrategy: this.getParameterValue(node, 'cacheKeyStrategy', 'input-hash')
      };
    }
    ...
    // Rate limiting
    const enableRateLimit = this.getParameterValue(node, 'enableRateLimit', false);
    if (enableRateLimit) {
      state['rateLimit'] = {
        enabled: true,
  ...
  override getDependencies(node: IRNode, _context: GenerationContext): string[] {
    ...
    const inputSchema = this.getParameterValue(node, 'inputSchema', {});
    const outputSchema = this.getParameterValue(node, 'outputSchema', {});
    if (Object.keys(inputSchema || {}).length > 0 || Object.keys(outputSchema || {}).length > 0) {
      deps.push('zod');
    }
  ...
  validateConfiguration(node: IRNode): { valid: boolean; errors: string[] } {
    ...
    // Check tool name
    const toolName = this.getParameterValue(node, 'name', null) as string | null;
    ...
    if (maxRetries !== null && (typeof maxRetries !== 'number' || maxRetries < 0 || maxRetries > 10)) {
...
export class CustomFunctionNodeConverter extends BaseAgentFlowV2Converter {
  ...
  protected override getAdditionalImports(node: IRNode, _context: GenerationContext): string[] {
    ...
    if (enableState) {
      imports.push('import { RunnablePassthrough } from "@langchain/core/runnables";');
    }
    
    // Add error handling imports
    const enableErrorHandling = this.getParameterValue(node, 'enableErrorHandling', true);
    if (enableErrorHandling) {
      imports.push('import { RunnableConfig } from "@langchain/core/runnables";');
    }
    
    // Add context imports
    const enableContext = this.getParameterValue(node, 'enableContext', false);
    if (enableContext) {
      imports.push('import { CallbackManagerForChainRun } from "@langchain/core/callbacks/manager";');
    }
  ...
  protected override generateSetupCode(
    ...
    if (enableState) {
      ...
      setupLines.push(`const ${variableName}_state = ${JSON.stringify(stateVariables, null, 2)};`);
    }
    
    // Setup error handling configuration
    const enableErrorHandling = this.getParameterValue(node, 'enableErrorHandling', true);
    if (enableErrorHandling) {
      const errorStrategy = this.getParameterValue(node, 'errorStrategy', 'throw') as string;
  ...
  protected extractNodeConfig(node: IRNode): Record<string, unknown> {
    ...
    if (functionCode) {
      ...
      if (enableLogging) {
        funcLines.push(`  console.log('[${name}] Function execution started', { input });`);
      }
      ...
      if (validateInput && Object.keys(inputSchema || {}).length > 0) {
        ...
        funcLines.push(`  } catch (error) {`);
        funcLines.push(`    throw new Error('Input validation failed: ' + error.message);`);
        funcLines.push(`  }`);
      }
      ...
      if (timeout && timeout > 0) {
        ...
        funcLines.push(`  const timeoutPromise = new Promise((_, reject) => {`);
        funcLines.push(`    setTimeout(() => reject(new Error('Function execution timeout')), ${timeout});`);
        funcLines.push(`  });`);
      }
      ...
      if (contextVars.length > 0) {
        funcLines.push(`    const userFunction = (input, ${contextVars.join(', ')}) => {`);
      } else {
      ...
      if (timeout && timeout > 0) {
        if (contextVars.length > 0) {
          ...
          funcLines.push(`      userFunction(input, ${contextVars.join(', ')}),`);
        ...
        if (contextVars.length > 0) {
          funcLines.push(`    const result = await userFunction(input, ${contextVars.join(', ')});`);
        } else {
      ...
      if (validateOutput && Object.keys(outputSchema || {}).length > 0) {
        funcLines.push(`    // Validate output`);
        funcLines.push(`    const validatedResult = ${name}_outputSchema.parse(result);`);
      }
      
      // Add metrics tracking
      if (enableMetrics) {
        funcLines.push(`    const executionTime = performance.now() - startTime;`);
        funcLines.push(`    console.log('[${name}] Execution metrics', { executionTime, success: true });`);
      }
      
      // Add success logging
      if (enableLogging) {
        funcLines.push(`    console.log('[${name}] Function execution completed', { result: ${validateOutput ? 'validatedResult' : 'result'} });`);
      }
      
      funcLines.push(`    return ${validateOutput && Object.keys(outputSchema || {}).length > 0 ? 'validatedResult' : 'result'};`);
      
      // Close error handling
      if (enableErrorHandling) {
        funcLines.push(`  } catch (error) {`);
        funcLines.push(`    console.error('[${name}] Function execution error:', error);`);
        
        if (errorStrategy && errorStrategy === 'fallback') {
          funcLines.push(`    if (${name}_errorConfig.fallbackValue !== null) {`);
  ...
  protected extractNodeState(node: IRNode): Record<string, unknown> {
    ...
    if (enableState || Object.keys(stateVariables || {}).length > 0) {
      ...
      if (persistState) {
        state['persist'] = true;
        state['persistKey'] = this.getParameterValue(node, 'persistKey', `custom_function_${node.id}`);
        state['persistTTL'] = this.getParameterValue(node, 'persistTTL', 86400000); // 24 hours
      }
    }
    ...
    if (enableMetrics) {
      state['metrics'] = {
        enabled: true,
        trackExecutionTime: true,
        trackMemoryUsage: this.getParameterValue(node, 'trackMemoryUsage', false),
        trackStateChanges: this.getParameterValue(node, 'trackStateChanges', enableState),
        trackErrorRate: true,
        aggregateMetrics: this.getParameterValue(node, 'aggregateMetrics', true)
      };
    }
  ...
  override canConvert(node: IRNode): boolean {
    ...
    const functionCode = this.getParameterValue(node, 'code', null) as string | null;
    
    return functionCode !== null && typeof functionCode === 'string' && functionCode.trim().length > 0;
  }
...
export class SubflowNodeConverter extends BaseAgentFlowV2Converter {
  ...
  protected override getAdditionalImports(node: IRNode, _context: GenerationContext): string[] {
    ...
    if (parallelExecution) {
      imports.push('import { RunnableParallel } from "@langchain/core/runnables";');
    }
    
    // Add conditional execution imports
    const enableConditional = this.getParameterValue(node, 'enableConditional', false);
    if (enableConditional) {
      imports.push('import { RunnableBranch } from "@langchain/core/runnables";');
    }
    
    // Add retry imports
    const enableRetry = this.getParameterValue(node, 'enableRetry', false);
    if (enableRetry) {
      imports.push('import { RetryOptions } from "@langchain/core/utils/async_caller";');
    }
  ...
  protected override generateSetupCode(
    ...
    if (Object.keys(inputMapping || {}).length > 0) {
      ...
      setupLines.push(`const ${variableName}_inputMapping = ${JSON.stringify(inputMapping, null, 2)};`);
    }
    
    if (Object.keys(outputMapping || {}).length > 0) {
      setupLines.push(`// Output mapping configuration`);
      setupLines.push(`const ${variableName}_outputMapping = ${JSON.stringify(outputMapping, null, 2)};`);
    }
    
    // Setup conditional execution
    const enableConditional = this.getParameterValue(node, 'enableConditional', false);
    if (enableConditional) {
      const conditionCode = this.getParameterValue(node, 'conditionCode', 'true');
  ...
  protected override generateInitializationCode(
    ...
    if (enableConditional) {
      ...
      initLines.push(`  [${variableName}_condition, /* CONDITIONAL_SUBFLOW_PLACEHOLDER */],`);
      ...
      if (parallelExecution) {
        ...
        config.steps.forEach((step: string, index: number) => {
          initLines.push(`  step${index}: ${step}${index < config.steps.length - 1 ? ',' : ''}`);
        });
        initLines.push(`});`);
      } else {
        initLines.push(`// Create sequential subflow execution`);
        initLines.push(`let ${variableName} = RunnableSequence.from([`);
        initLines.push(config.steps.map((step: string) => `  ${step}`).join(',\n'));
        initLines.push(`]);`);
      }
    } else {
      // Fallback to simple sequence
      initLines.push(`let ${variableName} = RunnableSequence.from([/* SUBFLOW_STEPS_PLACEHOLDER */]);`);
    }
    
    // Add error handling wrapper
    if (enableErrorHandling) {
      const errorStrategy = this.getParameterValue(node, 'errorStrategy', 'throw') as string;
      const fallbackSubflow = this.getParameterValue(node, 'fallbackSubflow', null);
    ...
    if (enableRetry) {
      ...
      initLines.push(`${variableName} = ${variableName}.withRetry(${variableName}_retryConfig);`);
    }
  ...
  override getDependencies(node: IRNode, _context: GenerationContext): string[] {
    ...
    const enableMonitoring = this.getParameterValue(node, 'enableMonitoring', false);
    if (enableMonitoring) {
      deps.push('@langchain/core/callbacks');
    }
  ...
  override canConvert(node: IRNode): boolean {
    ...
    const subflowId = this.getParameterValue(node, 'subflowId', null) as string | null;
    
    return subflowId !== null && typeof subflowId === 'string' && subflowId.trim().length > 0;
  }
  ...
  validateConfiguration(node: IRNode): { valid: boolean; errors: string[] } {
    ...
    // Check subflow ID
    const subflowId = this.getParameterValue(node, 'subflowId', null) as string | null;
    if (!subflowId || typeof subflowId !== 'string' || subflowId.trim().length === 0) {
    ...
    if (maxConcurrency !== null && (typeof maxConcurrency !== 'number' || maxConcurrency < 1 || maxConcurrency > 20)) {
      errors.push('maxConcurrency must be between 1 and 20 for parallel execution');
    }
