...
abstract class BaseLLMConverter extends BaseConverter {
  ...
  protected generateLLMConfiguration(node: IRNode, context: GenerationContext): {
    imports: string[];
    packageName: string;
    className: string;
    ...
    const temperature = this.getParameterValue<number>(node, 'temperature', 0.7);
  ...
  override convert(node: IRNode, context: GenerationContext): CodeFragment[] {
    ...
    const config = this.generateLLMConfiguration(node, context);
...
export class ChatOpenAIConverter extends BaseLLMConverter {
  ...
  protected override generateLLMConfiguration(node: IRNode, context: GenerationContext) {
    ...
    const frequencyPenalty = this.getParameterValue<number>(node, 'frequencyPenalty');
    const presencePenalty = this.getParameterValue<number>(node, 'presencePenalty');
...
export class AnthropicConverter extends BaseLLMConverter {
  ...
  protected override generateLLMConfiguration(node: IRNode, context: GenerationContext) {
    ...
    const model = this.getParameterValue<string>(node, 'modelName', 'claude-3-sonnet-20240229');
    const anthropicApiKey = this.getParameterValue<string>(node, 'anthropicApiKey');
    
    config.config = {
      ...(typeof model === 'string' && model ? { model } : {}),
      ...(config.config.temperature !== undefined ? { temperature: config.config.temperature } : {}),
      ...(config.config.maxTokens ? { maxTokens: config.config.maxTokens } : {}),
      ...(typeof anthropicApiKey === 'string' && anthropicApiKey ? { anthropicApiKey } : {})
    };
...
export class AzureOpenAIConverter extends BaseLLMConverter {
  ...
  protected override generateLLMConfiguration(node: IRNode, context: GenerationContext) {
    ...
    const azureOpenAIApiKey = this.getParameterValue<string>(node, 'azureOpenAIApiKey');
    const azureOpenAIApiVersion = this.getParameterValue<string>(node, 'azureOpenAIApiVersion');
    const azureOpenAIApiInstanceName = this.getParameterValue<string>(node, 'azureOpenAIApiInstanceName');
    const azureOpenAIApiDeploymentName = this.getParameterValue<string>(node, 'azureOpenAIApiDeploymentName');
...
export class OllamaConverter extends BaseLLMConverter {
  ...
  protected override generateLLMConfiguration(node: IRNode, context: GenerationContext) {
    ...
    const baseUrl = this.getParameterValue<string>(node, 'baseUrl', 'http://localhost:11434');
    ...
    config.config = {
      ...
      ...(config.config.temperature !== undefined ? { temperature: config.config.temperature } : {}),
      ...(config.config.maxTokens ? { numPredict: config.config.maxTokens } : {})
    };
...
export class HuggingFaceConverter extends BaseLLMConverter {
  ...
  protected override generateLLMConfiguration(node: IRNode, context: GenerationContext) {
    ...
    config.config = {
      ...
      ...(typeof endpointUrl === 'string' && endpointUrl ? { endpointUrl } : {})
    };
...
export class ReplicateConverter extends BaseLLMConverter {
  ...
  protected override generateLLMConfiguration(node: IRNode, context: GenerationContext) {
    ...
    const model = this.getParameterValue<string>(node, 'modelName', 'replicate/llama-2-70b-chat:latest');
