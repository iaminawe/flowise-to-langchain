...
abstract class BaseToolConverter extends BaseConverter {
  ...
  protected generateToolConfiguration(node: IRNode, context: GenerationContext): {
    imports: string[];
    packageName: string;
    className: string;
  ...
  convert(node: IRNode, context: GenerationContext): CodeFragment[] {
    ...
    const config = this.generateToolConfiguration(node, context);
...
export class SearchAPIConverter extends BaseToolConverter {
  ...
  convert(node: IRNode, context: GenerationContext): CodeFragment[] {
    ...
    const apiKey = this.getParameterValue(node, 'apiKey');
    const keyParam = apiKey ? this.formatParameterValue(apiKey) : 'process.env.SERPAPI_API_KEY';
...
export class WebBrowserConverter extends BaseToolConverter {
  ...
  protected extractToolConfig(node: IRNode): Record<string, unknown> {
    ...
    config.model = `\${${this.generateVariableName(node, 'llm')}}`;
    config.embeddings = `\${${this.generateVariableName(node, 'embeddings')}}`;
    
    const headless = this.getParameterValue(node, 'headless');
...
export class CustomToolConverter extends BaseToolConverter {
  ...
  protected extractToolConfig(node: IRNode): Record<string, unknown> {
    ...
    const name = this.getParameterValue(node, 'name');
    if (name) {
      config.name = name;
    }
    
    const description = this.getParameterValue(node, 'description');
    if (description) {
    ...
    const func = this.getParameterValue(node, 'func');
    if (func) {
      config.func = func;
    }
...
export class RequestToolConverter extends BaseToolConverter {
  ...
  convert(node: IRNode, context: GenerationContext): CodeFragment[] {
    ...
    const className = method?.toString().toUpperCase() === 'POST' ? 'RequestsPostTool' : 'RequestsGetTool';
...
export class FileSystemConverter extends BaseToolConverter {
  ...
  protected extractToolConfig(node: IRNode): Record<string, unknown> {
    ...
    const rootDir = this.getParameterValue(node, 'rootDir');
    if (rootDir) {
  ...
  private generateFileSystemConfigString(config: Record<string, unknown>): string {
    const entries = Object.entries(config);
    if (entries.length === 0) return '';
